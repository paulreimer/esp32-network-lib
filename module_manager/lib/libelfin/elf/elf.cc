        ...
        : m(make_shared<impl>(l))
{
        ...
        { printf("throw format_error(\"bad ELF magic number\");\n"); return; }
        if (core_hdr->ei_version != 1)
        { printf("throw format_error(\"unknown ELF version\");\n"); return; }
        if (core_hdr->ei_class != elfclass::_32 &&
            core_hdr->ei_class != elfclass::_64)
        { printf("throw format_error(\"bad ELF class\");\n"); return; }
        if (core_hdr->ei_data != elfdata::lsb &&
            core_hdr->ei_data != elfdata::msb)
        { printf("throw format_error(\"bad ELF data order\");\n"); return; }
        ...
        // More checks
        if (m->hdr.version != 1)
        { printf("throw format_error(\"bad section ELF version\");\n"); return; }
        if (m->hdr.shnum && m->hdr.shstrndx >= m->hdr.shnum)
        { printf("throw format_error(\"bad section name string table index\");\n"); return; }
...
section::as_strtab() const
{
        ...
        { printf("throw section_type_mismatch(\"cannot use section as strtab\");\n"); }
...
section::as_symtab() const
{
        ...
        { printf("throw section_type_mismatch(\"cannot use section as symtab\");\n"); }
...
strtab::get(Elf64::Off offset, size_t *len_out) const
{
        ...
        if (start >= m->end)
        { printf("throw range_error(\"string offset %s exceeds section size\");\n", std::to_string(offset).c_str()); return nullptr; }
        ...
        while (p < m->end && *p)
                p++;
        if (p == m->end)
        { printf("throw format_error(\"unterminated string\");\n"); return nullptr; }
